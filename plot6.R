...
# filtering Baltimore + Los Angeles data & group by Vehicule + year and calculate total emission

pack_NEI <- NEI_motor_id %>%
  filter(fips == "24510" | fips == "06037") %>%
  group_by(fips, Vehicule, year) %>%
  summarize(PM2.5_Emissions = sum(Emissions))
head(pack_NEI,10)

p <- ggplot(pack_NEI, aes(x = year, y = PM2.5_Emissions, color=as.factor(Vehicule))) + 
  facet_grid(.~fips) + geom_point() + geom_smooth(method="lm")
p + ggtitle("Motor vehicule PM2.5 Emissions") + ylab("PM2.5 Emissions (Tons)") + 
